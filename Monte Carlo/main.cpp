...
int main ()
{
  ...
  int N=128;  //create a 130*130 lattice instead of 128*128 to avoid margin effects
  int spin[N][N];
  double s=0.65,p;  //s is prob of having a +ve spin
  double deltaEij,deltaEkl,deltaE=0,prob,J;
  int xp,xn,yp,yn;
  ofstream magout,Eout,magorigout;
  magout.open("Magnetization0.8s10000.txt");  // Output file
  Eout.open("E0.8s10000.txt");
...
/*Set lattice points to 0
  ...
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      E[0]+=spin[i][j]*(spin[(i+N-1)%N][j]+spin[(i+1)%N][j]+spin[i][(j+N-1)%N]+spin[i][(j+1)%N]); //periodic boundary condition
      }
    }
    E[0]=abs(-J*E[0]);
    aveE[0]=E[0]/(N*N);
  ...
  for(a=0;a<steps;a++){
    ...
    for(t=0;t<N*N;t++){
      xp=int(RandomInt(0,N-1));//Put it your code here
      xn=int(RandomInt(0,N-1));
      yp=int(RandomInt(0,N-1));
      yn=int(RandomInt(0,N-1));
        if(spin[xp][xn]!=spin[yp][yn]){
          deltaEij=2*J*spin[xp][xn]*(spin[(xp+N-1)%N][xn]+spin[(xp+1)%N][xn]+spin[xp][(xn+N-1)%N]+spin[xp][(xn+1)%N]);//periodic boundary condition
          deltaEkl=2*J*spin[yp][yn]*(spin[(yp+N-1)%N][yn]+spin[(yp+1)%N][yn]+spin[yp][(yn+N-1)%N]+spin[yp][(yn+1)%N]);//periodic boundary condition
    ...
    for(i=0;i<N;i++){
      for(j=0;j<N;j++){
        E[a+1]+=spin[i][j]*(spin[(i+N-1)%N][j]+spin[(i+1)%N][j]+spin[i][(j+N-1)%N]+spin[i][(j+1)%N]);
      }
    }
    E[a+1]=abs(-J*E[a+1]);
    aveE[a+1]=E[a+1]/(N*N);
